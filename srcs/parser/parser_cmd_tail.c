...
/*   Updated: 2024/12/21 22:28:03 by ssoeno           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_node *node = create_node(ND_PIPE);
	if (!node)
	{
		free_node(left);
...
{
	t_node *node = create_logi_node(left, tk_list);
	if (!node)
	{
		free_node(left);
		d_throw_error("handle_logical_op", "failed to create logical operator node");
		return NULL;
	}
